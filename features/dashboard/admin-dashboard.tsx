...
function Brand() {
  return <Link href="/admin" className="flex items-center gap-3" aria-label="RioAnime admin"><span className="grid h-10 w-10 place-items-center rounded-[13px] bg-[#596fe5] text-[17px] font-bold text-white shadow-[0_8px_22px_rgba(89,111,229,0.28)]">R</span><span><span className="block text-[17px] font-bold tracking-[-0.02em] text-[#f5f7fb]">RioAnime</span><span className="block text-[10px] font-semibold uppercase tracking-[0.16em] text-[#7f899d]">Admin console</span></span></Link>;
}
...
function Overview({ data }: { data: DashboardResponse | null }) {
  return <><OverviewAnalytics />{data ? <section className="mt-5 grid gap-5 xl:grid-cols-[1.3fr_0.7fr]"><RecentContent data={data} compact /><ActivityList data={data} compact /></section> : null}</>;
}
...
function SettingsPanel({ data, appearance, initialProfile, onAppearanceChange }: { data: DashboardResponse | null; appearance: AdminAppearance; initialProfile: Profile | null; onAppearanceChange: (appearance: AdminAppearance) => void }) {
  return <div className="space-y-5"><AccountSettings initialProfile={initialProfile} /><AdminAppearanceSettings appearance={appearance} onChange={onAppearanceChange} /><section className="admin-card p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--admin-accent-soft)] text-[var(--admin-accent-text)]"><MaterialIcon name="dns" /></span><div><h2 className="text-sm font-bold text-[var(--admin-text)]">Platform status</h2><p className="admin-support mt-0.5">Live infrastructure connection details</p></div></div><dl className="mt-5 grid gap-3 text-xs sm:grid-cols-2"><div className="rounded-xl border border-[var(--admin-border)] bg-[var(--admin-input)] p-4"><dt className="text-[var(--admin-subtle)]">API status</dt><dd className="mt-1.5 flex items-center gap-2 font-semibold capitalize text-[var(--admin-success)]"><span className="h-1.5 w-1.5 rounded-full bg-current" />{data?.health.api ?? "Unavailable"}</dd></div><div className="rounded-xl border border-[var(--admin-border)] bg-[var(--admin-input)] p-4"><dt className="text-[var(--admin-subtle)]">Database status</dt><dd className="mt-1.5 flex items-center gap-2 font-semibold capitalize text-[var(--admin-success)]"><span className="h-1.5 w-1.5 rounded-full bg-current" />{data?.health.database ?? "Unavailable"}</dd></div></dl><div className="mt-4 overflow-hidden rounded-xl border border-[var(--admin-border)] bg-[var(--admin-input)]"><div className="flex items-center justify-between border-b border-[var(--admin-border)] px-4 py-3"><p className="text-xs font-semibold text-[var(--admin-text)]">Tracked routes</p><span className="text-[10px] text-[var(--admin-subtle)]">{data?.routeMetrics.length ?? 0} configured</span></div><div className="grid gap-px bg-[var(--admin-border)] sm:grid-cols-2">{data?.routeMetrics.map((metric) => <div key={metric.route} className="flex items-center justify-between gap-3 bg-[var(--admin-surface)] px-4 py-3"><code className="min-w-0 truncate text-[10px] text-[var(--admin-accent-text)]">{metric.route}</code><span className="shrink-0 text-[9px] text-[var(--admin-subtle)]">{metric.requests} requests / 24h</span></div>)}</div></div></section></div>;
}
