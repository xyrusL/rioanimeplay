...
export function MobileBottomNav() {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center px-6 pb-[max(1.15rem,calc(env(safe-area-inset-bottom)+0.6rem))]">
      <nav
        aria-label="Primary"
        className="pointer-events-auto flex w-full max-w-[332px] items-center justify-between rounded-[999px] border border-[var(--line-soft)] bg-[color:color-mix(in_srgb,var(--bg-elevated)_90%,transparent)] px-2.5 py-2 shadow-[0_22px_54px_rgba(0,0,0,0.4),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl"
      >
        {MOBILE_NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span
                className={`flex h-[52px] w-[52px] items-center justify-center rounded-full border transition-[background-color,border-color,box-shadow,transform] duration-[var(--motion-base)] ease-[var(--ease-smooth)] ${
                  active
              ...
              >
                <MaterialIcon
                  className={active ? "text-[24px]" : "text-[23px]"}
                  filled={active}
                  name={item.icon}
                />
              </span>
