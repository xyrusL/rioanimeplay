...
export function MobilePosterCard({ item, className }: MobilePosterCardProps) {
  return (
    <Link
      href={`/watch/${toAnimeSlug(item.title)}`}
      className={`group flex min-h-[292px] flex-col overflow-hidden rounded-[24px] border border-[var(--line-soft)] bg-[var(--bg-card)] shadow-[var(--soft-shadow)] ${className ?? "w-[148px] shrink-0"}`}
    >
      <div className="relative aspect-[11/16] shrink-0 overflow-hidden">
        <Image
          fill
          alt={item.title}
      ...
      <div className="flex min-h-[82px] flex-1 flex-col justify-between gap-2 px-3 py-3">
        <h3 className="line-clamp-2 min-h-[2.5rem] text-sm font-semibold leading-5 text-[var(--text-primary)]">
          {item.title}
        </h3>
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[68px] w-[52px] shrink-0 overflow-hidden rounded-[16px]">
        <Image fill alt={entry.title} className="object-cover" sizes="52px" src={entry.image} />
      </div>
